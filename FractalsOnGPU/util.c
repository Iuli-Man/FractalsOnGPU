...
void description(char* name, char* desc, int blocks, int threads) {
    if (strcmp("gpu", name) == 0) {
        sprintf(desc, "width=%d height=%d iterations=%d work_groups=%d work_items=%d", WIDTH, HEIGHT, MAX_ITERATION, blocks, threads);
    }
}

void progress(char* name, int r, double time, int blocks, int threads) {
    char desc[100];
    description(name, desc, blocks, threads);
...
void report(char* name, double* times, float avg_blocks, float avg_threads) {
    ...
    sprintf(rep, "name=%s %s min=%.2lf max=%.2lf mean=%.2lf avg=%.2lf stdev=%.2lf avg blocks=%.2lf avg threads=%.2lf\n", name, desc, min, max, mean, avg, stdev, avg_blocks, avg_threads);
